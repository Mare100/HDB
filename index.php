...
//exit;
  ...
  while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
      ...
      $tier['TierRasse'] = isset($row['HDB_id']) ? $row['HDB_id'] : null;
...
/* =========================================================
 * FIX für HDB-Schema: Arrays → stdClass
 * (FUNKTION MUSS AUSSERHALB DER WHILE-SCHLEIFE STEHEN)
 * ========================================================= */


/* =========================================================
 * SOAP-Parameter korrekt aufbauen
 * ========================================================= */
...
// HE MUSS Liste sein
if (isset($halter['HE']) && array_keys($halter['HE']) !== range(0, count($halter['HE']) - 1)) {
    $halter['HE'] = [ $halter['HE'] ];
}
...
// HalterEigentuemer korrekt wrappen
$heWrapper = new stdClass();
$heWrapper->HE = $halter->HE;

// params laut WSDL
$params = new stdClass();
$params->Tier = $tier;
$params->HalterEigentuemer = $heWrapper;
...
if ($response->Status == 0) {
    ...
    mysql_query(
        "DELETE FROM `i_heimtierdb_at:error`
         WHERE `transponder` = '".mysql_real_escape_string($row['transponder'])."'",
        $conn
    );

} elseif ($response->Status == 200) {

    $errormsg = "ChipCode \"".$row['transponder']."\" existiert bereits!\n";
    print $errormsg;

} else {

    $errormsg =
        "Fehler bei Erstmeldung (ChipCode \"".$row['transponder']."\"):\n".
        '$response = '.print_r($response,true).
        '$tier = '.print_r($tier,true).
        '$halter = '.print_r($halter,true);
...
$sql = sprintf(
    ...
    mysql_real_escape_string(serialize([$tier, $halter])),
    $response_time,
    $response->Status,
